...
import { CheckCircle, Download, Share, Calendar, MapPin, Users, CreditCard, Phone } from "lucide-react"
...
const BookingConfirmation: React.FC = () => {
  ...
  const { selectedSchedule, selectedSeats, passengers, totalAmount, paymentMethod } = useSelector(
  ...
  useEffect(() => {
    if (!bookingId || !selectedSchedule) {
      navigate("/")
    }
  }, [bookingId, selectedSchedule, navigate])
  ...
  const handleShareTicket = () => {
    ...
    if (navigator.share) {
      navigator.share({
        title: "E-Ticket Bus",
        text: `Tiket bus saya: ${selectedSchedule?.Route?.originTerminal?.city} → ${selectedSchedule?.Route?.destinationTerminal?.city}`,
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="space-y-4">
              <div className="bg-primary-50 rounded-lg p-4">
                <div className="text-center">
                  ...
                  <p className="text-2xl font-bold text-primary-600 font-mono">{bookingId}</p>
                </div>
              </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <button onClick={handleDownloadTicket} className="btn btn-outline">
            <Download size={20} />
            Download E-Ticket
          </button>
